...
# NEW: Role variable
var my_role: String = ""
# NEW: mark if this player is the Leader
...
func set_role(new_role: String):
	...
	if is_multiplayer_authority():
		...
		if multiplayer.is_server():
			reveal_scene.role_reveal_finished.connect(
				Callable(get_tree().get_root().get_node("GameManager"), "on_role_reveal_finished")
			)
	else:
		if role_label:
			role_label.text = ""
...
# Called when server chooses Leader
func set_leader_local():
	is_leader = true
	if is_multiplayer_authority():  # Only you see your own Leader role
		if role_label:
