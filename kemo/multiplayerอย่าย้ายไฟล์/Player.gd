...
func set_role(new_role: String, is_leader: bool = false):
	...
	# Save leader status in Global
	if is_leader:
		Global.leader_id = get_multiplayer_authority()
	
	# Show private role for local player
	if is_multiplayer_authority():
		if role_label:
			var private_text = "Role: " + new_role
			if is_leader:
				private_text += " (Leader 👑)"  # Add leader tag to base role
	...
	# Always update public visibility
	update_role_visibility()


# ⭐️ NEW FUNCTION to handle public role visibility
func update_role_visibility():
	# Only the Leader role is publicly visible to all players
	if Global.leader_id == get_multiplayer_authority():
		role_label.text = "👑 Leader"
		role_label.modulate = Color.YELLOW
		role_label.visible = true
	else:
		# Hide the role label for all other players
		role_label.visible = false
	
	# A player's own private role is handled by the `set_role` function
	if is_multiplayer_authority() and role_label:
		role_label.visible = true
...
# This function is no longer needed since we handle it in `update_role_visibility`
# func set_leader_local():
#     role_label.text = "👑 Leader"
#     role_label.visible = true # Leader is visible to *all* players
