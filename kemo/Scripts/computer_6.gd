...
var computer_id = 6 # เปลี่ยนค่านี้สำหรับแต่ละคอมพิวเตอร์ (1-6) [cite: 4]
...
func _on_body_entered(body):
    # ตรวจสอบว่าเป็นผู้เล่นหรือไม่ และผู้เล่นคนนั้นเป็นผู้เล่นของเครื่องนี้หรือไม่
...
func _on_body_exited(body):
    # ทำการซ่อน UI เฉพาะเมื่อผู้เล่นในเครื่องนี้เดินออกจากพื้นที่
    if body.is_in_group("players") and body.get_multiplayer_authority() == multiplayer.get_unique_id():
        ui_enter_password.hide()
