...
func process_card_collection(card_path: String, peer_id: int):
    ...
    # This call is still useful for updating the local player's UI directly.
    rpc_id(peer_id, "update_cards_ui_for_peer", player_node.collected_cards.size())

# ⭐ NEW: A new RPC function to synchronize card data across all peers.
@rpc("any_peer", "reliable", "call_local")
func sync_card_collection(collected_cards_dict: Dictionary):
